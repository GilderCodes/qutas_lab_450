...
AreaMap::AreaMap() :
	...
	// Generate random obstacle regardless of other obstacles
	if(true) {
		srand(time(NULL));  // Seed the random generator

		obstacles_t obs;
		obs.type = OBS_SQUARE;
		obs.size = 3;  // Default size

		// Load random obstacle parameters from the YAML file
		int axes_div = 6;
		nh_.param("obstacles/rand/size", obs.size, obs.size);  // Load size from YAML
		nh_.param("obstacles/rand/divisor", axes_div, axes_div);  // Load divisor from YAML

		// Randomize position within the map bounds
		...
		// Adjust position to center in the map
		obs.x += param_map_width_ / 2;
		obs.y += param_map_height_ / 2;

		ROS_INFO("Placing random obstacle (size: %i; divisor: %i; p: %0.2f)", obs.size, axes_div, p);
		obs_.push_back(obs);
	}
        ...
        case OBS_SQUARE: {
            for (int y = -obs_[k].size; y < obs_[k].size + 1; y++) {
                for (int x = -obs_[k].size; x < obs_[k].size + 1; x++) {
                    ...
                    if ((dx >= 0) && (dx < param_map_width_) &&
                        (dy >= 0) && (dy < param_map_height_)) {
